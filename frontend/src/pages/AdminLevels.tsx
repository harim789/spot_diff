...
export default function AdminLevels() {
  ...
  const leftPreview = useMemo(
    () => (leftFile ? URL.createObjectURL(leftFile) : ""),
    [leftFile],
  );
  const rightPreview = useMemo(
    () => (rightFile ? URL.createObjectURL(rightFile) : ""),
    [rightFile],
  );
  ...
  const createLevel = async () => {
    ...
    if (!leftFile || !rightFile)
      return setErr("left/right image files are required");
          ...
          {levels.map((lv) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <div>
                  <Link
                    to={`/admin/levels/${lv.id}/diffs`}
                    className="rounded-xl border px-3 py-1.5 mr-3 text-sm hover:bg-white"
                  >
                    Diff 편집
                  </Link>
                  <button
